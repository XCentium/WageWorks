...
var publishStream = function(stream, dest) {
    var targets = ["Build"];
    console.log("publishStream dest: " + dest);
    return stream
...
gulp.task("Auto-Publish-Assemblies",
    function() {
        ...
        var files = "/**/Vista.{Feature,Foundation,Project}.*.{dll,pdb}";;
        var destination = config.websiteRoot + "/bin/";
        gulp.src(roots, { base: root }).pipe(
            foreach(function(stream, rootFolder) {
                gulp.watch(rootFolder.path + files,
...
gulp.task('reset-apppool', function () {
    return run('%systemroot%\\system32\\inetsrv\\appcmd recycle apppool /apppool.name:' + config.appPoolName).exec();
});