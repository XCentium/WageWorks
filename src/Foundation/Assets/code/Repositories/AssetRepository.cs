...
namespace Wageworks.Foundation.Assets.Repositories
{
    ...
    public class AssetRepository
    {
            ...
            if (!preventAddToCache)
            {
                if (RenderingContext.CurrentOrNull != null)
                {
                    ...
                    if (rendering != null && rendering.Caching.Cacheable)
                    {
                        ...
                        var cacheKey = $"{renderingId}-{Sitecore.Context.Site?.Name}";
                        ...
                        {
                            cachedRequirements = _cache.Get(cacheKey) ?? new AssetRequirementList();
                        }

                        cachedRequirements.Add(asset);
                        _cache.Set(cacheKey, cachedRequirements);
                    }
                }
            }
        ...
        public Asset AddInlineScript(string script, ScriptLocation location, bool preventAddToCache, bool useDocumentReady)
        {
            return this.AddAsset(script, location, preventAddToCache, AssetType.JavaScript, AssetContentType.Inline, null, useDocumentReady);
        }
        ...
        private Asset AddAsset(string content, ScriptLocation location, bool preventAddToCache, AssetType assetType, AssetContentType contentType, string site = null, bool useDocumentReady = true)
        {
            var asset = this.CreateAsset(content, location, assetType, contentType, site);
            if (asset != null)
            {
                asset.UseDocumentReady = useDocumentReady;
            }
