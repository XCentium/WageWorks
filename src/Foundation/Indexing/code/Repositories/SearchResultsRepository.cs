...
namespace Wageworks.Foundation.Indexing.Repositories
{
    ...
    public class SearchResultsFactory
    {
        ...
        public ISearchResults Create(SearchResults<SearchResultItem> results, IQuery query, bool distinct=true)
        {
            var searchResults = this.CreateSearchResults(results, distinct);
        ...
        public ISearchResults CreateSpatial(SearchResults<SpatialSearchResultItem> results, ISpatialQuery query)
        {
            var searchResults = this.CreateSpatialSearchResults(results);
            //var facets = this.CreateSpatialFacets(results, query).ToArray();
            return new SearchResults
            {
                Results = searchResults,
                TotalNumberOfResults = results.TotalSearchResults,
		...
		private IEnumerable<ISearchResultFacet> CreateSpatialFacets(SearchResults<SpatialSearchResultItem> results, IQuery query)
        {
            if (results.Facets == null)
                yield break;
        ...
        {
            return IndexingProviderRepository.QueryFacetProviders.SelectMany(provider => provider.GetFacets())
        ...
        private IEnumerable<ISearchResult> CreateSearchResults(SearchResults<SearchResultItem> results, bool distinctoption = true)
        {
            if (distinctoption)
            {
                return results.Hits.Select(h => SearchResultFactory.Create(h.Document)).Distinct(new SearchResultEqualityComparer()).ToArray();
            }
            else {
        ...
        private IEnumerable<ISearchResult> CreateSpatialSearchResults(SearchResults<SpatialSearchResultItem> results)
        {
            return results.Hits.Select(h => SearchResultFactory.Create(h.Document)).ToArray();
        }
    }
